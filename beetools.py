...
class DrawingTool(AbstractTool):
	...
	def makeFullSizedBrush(self):
		...
		self.brushimageside=side
		center=self.radius-1
		self.fullsizedbrush=qtgui.QImage(side,side,qtgui.QImage.Format_ARGB32_Premultiplied)
	...
	def updateBrushForPressure(self,pressure):
		...
		bradius=self.options["radius"]*pressure
		bside=bradius*2

		# bounding radius for pixels to update
		self.radius=int(math.ceil(bradius))
		side=(self.radius*2)+1

		if self.radius==0:
			self.radius=1

		fullsizedrect=qtcore.QRectF(0,0,self.brushimageside,self.brushimageside)
		cursizerect=qtcore.QRectF(0,0,bside,bside)
		...
		for i in range(side):
			for j in range(side):
				...
				if distance < self.radius-.5:
					...
					fade=(1-(distance/(self.radius-.5)))/(blur/100.0)
					if fade > 1:
						fade=1
...
class PencilToolDesc(AbstractToolDesc):
	...
	def runOptionsDialog(self,parent):
		...
		dialog.ui.brushradius.setValue(self.options["radius"])
		...
		if dialog.result():
			self.options["radius"]=dialog.ui.brushradius.value()
...
class EraserToolDesc(AbstractToolDesc):
	...
	def runOptionsDialog(self,parent):
		...
		dialog.ui.eraserradius.setValue(self.options["radius"])
		dialog.ui.stepsize.setValue(self.options["step"])
